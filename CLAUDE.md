...
- **Tab reading** — `src/learning/tabItem.ts` / `tabDrill.ts` (items keyed `tab:<string>:<fret>` — a place, not a pitch; pool per fret range, SRS-weighted picker and a riff builder that stays under one hand), `tabMastery.ts`, the shared `useReadingEngine` + `TabPracticeScreen` (four exercises: name the note / find that exact place on the neck / write a marked place in tab / read a riff; its own `tabDaily` goal and `TabTodayCard`). `TabNotation.tsx` draws the tab with **string 1 (the thinnest) on the top line**, like every real tab — a deliberate, owner-approved exception to the app's lowest-string-on-top necks, explained on screen; the neck boards beside it keep the app's layout. See `.kiro/specs/roadmap/tab-reading-spec.md`.
