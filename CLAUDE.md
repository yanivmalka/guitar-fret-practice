...
`src/i18n/` — a lightweight in-house system, no library. The **English source string is the lookup key**: call sites wrap literals in `t('English text')`; missing entries (and English) fall back to the literal. Hebrew (`he`) is a full **RTL** translation; Spanish (`es`, `translations.es.ts`), Brazilian Portuguese (`pt-BR`, `translations.ptBR.ts`) and French (`fr`, `translations.fr.ts`) are LTR and partial — they land in stages (see the wishlist) and currently cover the same screens with the same key set, so a new string needs a `he` entry and, when it belongs to an already-translated screen, an `es`, a `pt-BR` and an `fr` entry too. Anything that switches on `lang` must handle all five (`isLang`, `dateLocale`); Spanish, Portuguese and French default note names to Do-Re-Mi, and new layout must work in both directions (see the direction-driven chevrons / nav rows). Some deeper Game/stage keys are intentionally rendered raw for now.
