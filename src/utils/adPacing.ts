...
//
// The rule (product owner): an ad line shows up after a random 1–3 rounds —
// finished or stopped part-way, both count — and only while the user is
// browsing, never during a drill. Every drill engine reports two moments here:
// a round ending, naturally or by the user stopping it (`noteRoundEnded`), and
// a round being started (`noteRoundStarted`). The
...
// (or when the user closes it). Each cycle then draws a fresh 1–3 threshold.
//
...
/** A round ended — ran to its natural end or was stopped part-way by the user.
 *  Raises the strip once the count reaches the current threshold. */
