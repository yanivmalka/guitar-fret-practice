...
- **Audio refinements** — single-note question sound, satisfying correct chime, escalating streak tone, optional background beats toggle. Still only the first two exist; `playStreakTone` and the background-beats toggle described below (B1/B2) are **not built**. B3 (Silent Mode) split off from this item and **is** built — see below.
...
- **Left-handed fretboard mode.** A setting that mirrors the fretboard for left-handed players — nut on the right, high frets to the left, and the string order flipped so the layout matches a left-handed instrument held the mirror way round. This is **orientation of the instrument, not text direction**: the app already lays itself out RTL for Hebrew, but that flips reading order and nav chrome, not the fretboard's low-string/nut geometry, so the two are independent axes and a left-handed English user or a right-handed Hebrew user must each get the orientation they expect. Touch points: `FretGrid` (fret/string rendering + the tap targets in "by note" mode), the fret highlight in "by fret" mode, any fret-number ruler, and possibly `NoteCircle` (though the circle of fifths has no handedness — likely left alone). `src/utils/instruments.ts` string ordering and the mastery overlay (`src/utils/mastery.ts`) coordinates are unaffected — this is a pure render-time transform keyed on one new `pref_leftHanded` boolean, not a data change, so history and sync stay identical across the two modes. Not built; no mirroring code exists. Free for everyone (an accessibility affordance, not a paid feature). Needs a product-owner decision on whether it is wanted and where the toggle lives.
