...
- **Left-handed mode.** ~~A setting that mirrors the fretboard for left-handed players.~~ **Built (2026-09-10)**, and wider in scope than this entry first assumed. The product owner was explicit that Hebrew RTL only changes *reading direction*, not the physical position of chrome (menu, Quick Access, back buttons all stay on the right in both languages), so left-handed is a genuinely independent axis that mirrors the instrument **and** moves that chrome to the left. Shipped as `pref_leftHanded` → `data-hand="left"` on `<html>` (separate from `dir`), synced via `settingsSync`, toggle in Settings → General. Fretboard: the by-note tap grid reverses via `direction: rtl` on `.fret-grid`; the little SVG necks flip via a CSS `scaleX(-1)` with counter-flipped text. Chrome: `27-left-handed.css` overrides the physical side of the hamburger / stats buttons, Quick Access (plus its summon quadrant and hint, handled in `QuickAccess.tsx`), the debug FAB and the mode-info bubble; the Settings-drawer back rows / chevrons mirror relative to the language. Right-handed is byte-for-byte unchanged (every rule scoped to `[data-hand="left"]`). `NoteCircle`, `instruments.ts` string ordering and the mastery overlay are untouched — pure render-time transform, so history/sync are identical in both modes. **Still needs a real visual pass** in the browser in both languages: the SVG-neck text counter-flip (`transform-box: fill-box`), the Pro fret-range drag slider under `scaleX(-1)`, and the drawer `direction` flips were not verified on a device.
