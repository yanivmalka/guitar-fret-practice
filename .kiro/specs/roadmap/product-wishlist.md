...
- **Staff reading — Slice 1 shipped 2026-09-25; later slices not built.** Design and status in `staff-reading-spec.md`. Built: a Premium `Learn` → `Staff reading` screen with two exercises (name the written note / find it on the neck), three fret ranges (0–3, 0–5, 0–12), naturals or all twelve notes, the right clef per instrument (guitar/banjo treble 8vb, bass clef 8vb, mandolin/ukulele treble at pitch), and its own local SRS lane (`staffSrs` / `staffHistory`). **Deferred, in priority order (spec §11):** a Progress tab; cloud sync of the staff fields (together with the scale fields); a reverse exercise (a place on the neck is marked → tap where it is written); reading a short phrase of 4–8 notes; positions beyond fret 12 and key signatures; a staff daily goal / Teacher integration.
