...
- **More UI languages** — requested 2026-09-25; recommended order Spanish → Portuguese (Brazil) → German/French/Italian/Japanese (German needs an H-notation mode). Not built; detail in §3 OPEN (continued).
...
- **More UI languages (beyond English + Hebrew)** — requested 2026-09-25. Today `Lang` in `src/i18n/translations.ts` is `'en' | 'he'`; adding a locale means a new dictionary (~900 entries keyed by the English source string) plus a `LANGUAGES` row. Missing entries fall back to English, so a language can land in stages (main screens first, then settings/Learn, then the rest). **Recommended order (not yet confirmed by the owner):**
  1. **Spanish** — first. Large guitar market (Spain + Latin America, strong classical-guitar culture); solfège (Do Re Mi … Si) is already a supported notation (`NotationMode 'solfege'` in `src/utils/music.ts`), so it only needs to become the default notation for this language; left-to-right, so no layout work.
